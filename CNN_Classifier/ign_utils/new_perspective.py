...
class Caliberate():
	def __init__(self, recaliberate=True, dst_folder=None):
		self.imageWindowName   = 'image'
		self.birdeyeWindowName = 'BirdEye'
		self.cropWindowName    = 'CropImage'
		if dst_folder is not None:
			self.pointssaveFile    = osp.join(dst_folder, 'points.txt')
			self.WarpMatrixFile    = osp.join(dst_folder, 'warpmatrix.txt')
			self.ReferenceptsFile  = osp.join(dst_folder, 'refpts.txt')
			self.ROIptsFile        = osp.join(dst_folder, 'roipts.txt')
			self.InvWarpMatrixFile = osp.join(dst_folder, 'inv_warpmatrix.txt')
		self.recaliberate      = recaliberate
		self.x_offset, self.y_offset = 0, 0
		
		if not self.recaliberate:
			if osp.isfile(self.pointssaveFile):
	...
	def CaliberateImage(self, image, num_pts=4):
		...
		return M, M_inv, transformed_two_pts, roipts, inv_roipts
...
if __name__=='__main__':
	if 1:
		Cal_Obj = Caliberate(recaliberate=True, dst_folder='/home/ignitarium/projects/Social2/caliberated_data/test')
		image = cv2.imread('/home/ignitarium/projects/Social2/sample_video/virat_sample1.png')
		Cal_Obj.CaliberateImage(image)
	
	if 0:
		Cal_Obj = Caliberate(recaliberate=False)
		print(osp.join(path, 'data', 'newsample.png'))
		image = cv2.imread(osp.join(path, 'data', 'newsample.png'))
